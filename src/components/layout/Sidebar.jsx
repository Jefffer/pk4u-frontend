...
import { LiaMapMarkerSolid, LiaCarSideSolid, LiaLayerGroupSolid } from "react-icons/lia";
...
const Sidebar = ({ selectedParkingId, searchInputRef, onAnimationComplete }) => {
  ...
  return (
      ...
      >
        ...
        {error && (
            <p className="text-red-500 dark:text-red-400 mt-4">{t("error.loadDetails")}</p>
        )}
        ...
        {parkingDetails && !isLoading && !error && (
            <div className="mt-6 space-y-5">
              {!imageError && imageUrl && (
                  <img
                      src={imageUrl}
                      alt={t("image.alt", { name: parkingDetails.name })}
              ...
              {imageError && (
                  <div className="w-full h-32 bg-slate-200 dark:bg-slate-700 flex flex-col items-center justify-center rounded-lg mb-4 text-slate-500 dark:text-slate-400 text-sm shadow">
                    ...
                    <span>{t("image.notAvailable")}</span>
                  </div>
              )}
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <span className="font-semibold text-slate-800 dark:text-slate-100">
                    {parkingDetails.numLevels}
                      ...
                      <span className="ml-1">{t("levels")}</span>
                    </div>
                  </div>
                  <div className="flex items-center p-3 bg-slate-100 dark:bg-slate-700/50 rounded-lg shadow-sm">
                  ...
                  <span className="font-semibold text-slate-800 dark:text-slate-100">
                    {parkingDetails.totalSpots}
                      ...
                      <span className="ml-1">{t("totalSpots")}</span>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <ParkingOccupacy total={parkingDetails.totalSpots} rate={parkingDetails.rate}>
                    <ul className="space-y-3">
                      {parkingDetails.levelsInfo.map((level) => {
                        const colorClass = getAvailabilityColor(level.spotsFree, level.spotsTotal);
                        const IconComponent = getAvailabilityIcon(level.spotsFree, level.spotsTotal);
                        const percentageFull =
                            level.spotsTotal > 0
                                ? Math.round(100 - (level.spotsFree / level.spotsTotal) * 100)
                                : 0;
                            ...
                            >
                              <div className="flex items-center justify-between mb-1">
                                <div className="flex items-center">
                                  ...
                                  <span className="font-semibold text-md">{level.levelName}</span>
                                </div>
                                <span className="text-sm font-medium">
                          {level.spotsFree} / {level.spotsTotal} {t("free")}
                        </span>
                              </div>
        ...
        {!parkingDetails && searchTerm && (
            <p className="text-slate-600 dark:text-slate-400 mt-4">
              {t("searchResults", { term: searchTerm })}
            </p>
        )}
